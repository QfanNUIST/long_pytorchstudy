...
model.device = device
# m_state_dict = torch.load('modetransl_before_split-validation_acc_0.7130.pth')  # 下载存储的模型参数
...
for epoch in range(epoches):
    ...
    """visdom 可视化"""
    '''loss可视化'''
    viz.line([[train_epoch_loss,valid_epoch_loss]], [epoch], win='loss', update='append')
    viz.line([[train_epoch_acc.cpu(),valid_epoch_acc.cpu()]], [epoch], win='acc', update='append')

    if valid_epoch_acc > valid_acc_max:
        valid_acc_max = valid_epoch_acc
        print("valid_acc_max:[{}:.4f]".format(valid_acc_max))
        model_name = 'model-3-classify-validation_acc_max.pth'
